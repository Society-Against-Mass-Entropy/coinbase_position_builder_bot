...
module.exports = (data) => {
  ...
  log.now(
    `${emoji} ${logData.Time} ${logData.Funds > 0 ? "bought" : "sold"} ${
      logData.Funds
    } ${config.currency} worth of ${config.ticker} @ ${logData.Price} = ${
      logData.Shares
    }, total accumulation = ${logData.Holding} with liquid profit ${
      logData.Profit
    }`
  );
