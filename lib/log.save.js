...
module.exports = (data) => {
  ...
  log.now(
    ...
    }, ✊ ${logData.Holding} @ ${divide(action.totalInput, logData.Holding).toFixed(2)}, 💧 basis ${divide(subtract(action.totalInput, action.realized), logData.Holding).toFixed(2)}, 💵 ${action.realized}, 💹 ${logData.Profit > 0 ? '+' : ''}${logData.Profit} `
  );
