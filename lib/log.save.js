  ...
  log.now(
    ...
    logData.Funds > 0
      ? `${Math.abs(logData.Funds)} ${config.currency} ➡️  ${Math.abs(
        logData.Shares
      )} ${config.ticker} @ ${numFix(logData.Price, 2)}`
      : `${Math.abs(logData.Shares)} ${config.ticker} ➡️  ${Math.abs(
        logData.Funds
      )} ${config.currency} @ ${NumFix(logData.Price, 2)}`
    }, ✊ ${logData.Holding} @ ${numFix(divide(action.totalInput, logData.Holding), 2)}, 💧 basis ${numFix(divide(subtract(action.totalInput, action.realized), logData.Holding), 2)}, 💵 ${action.realized}, 💹 ${logData.Profit > 0 ? '+' : ''}${logData.Profit} `
  );
