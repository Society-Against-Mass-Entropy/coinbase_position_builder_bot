...
module.exports = ({ action, order }) => {
  ...
  // correct funds to filled values (including fees)
  action.funds = sold
    ? multiply(-1, subtract(order.filled_value, order.total_fees))
    : add(order.filled_value, order.total_fees);
  ...
  const logData = {
    ...
    Funds: action.funds,
    Shares: multiply(order.filled_size, !sold ? 1 : -1),
