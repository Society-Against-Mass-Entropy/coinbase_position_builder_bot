...
const { divide, multiply, subtract } = require('./math');
...
module.exports = data => {
  ...
  const buyBelow = data.Target
    ...
    : Infinity;
  log.now(
    `${emoji} ${moment(data.Time).local().format()} ${data.Method} ${
      data.Funds > 0
      ...
      ` 💧 gain ${data.Liquid.toFixed(2)}, basis ${liquidBasis.toFixed(
        ...
        process.env.CPBB_APY_DYNAMIC
          ? ` | ${multiply(data.TargetAPY || 0, 100, 4)}% TAPY`
          : ''
      }`
  );
};
