...
const { divide, subtract } = require('mathjs');
...
module.exports = (logData) => {
  ...
  log.now(
    ...
    } ✊ ${currentHolding.toFixed(8)} @ ${costBasis} 💧 gain ${logData.Liquid} 💧 basis ${liquidBasis} 💵 ${logData.Realized.toFixed(2)} 💹 ${logData.Profit > 0 ? '+' : ''}${logData.Profit} 💸 <${buyBelow}`
  );
}
