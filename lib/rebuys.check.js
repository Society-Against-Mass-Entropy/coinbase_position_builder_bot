...
module.exports = async () => {
  ...
  for (let i = 0; i < pendingOrders.length; i++) {
    ...
    if (!json && res && res.statusCode === 404) {
      log.now(
        `🗑️ limit order ${order.id} not found (${res.statusCode}) (must have been deleted manually or through Coinbase maintenance), removing from tracking db.`
      );
      memory.makerOrders.orders = memory.makerOrders.orders.filter(
        o => o.id !== order.id
      );
      ...
      let funds = add(json.executed_value, json.fill_fees);
      let action = await calculateAction({
        dateOverride: new Date(json.done_at),
        forceBuy: true,
      ...
      log.debug(`limit order filled!`, { order, json, action });
      logSave({
        action,
        order: json,
      });
      ...
      if (killAfter <= now) {
        ...
        log.now(
          `⏰ limit ${order.id} has more time: ${order.size} ${
            config.ticker
          } @ ${order.price} = $${order.funds} | expires in ${moment
    ...
    await sleep(config.sleep.rebuyCheck);
  }
