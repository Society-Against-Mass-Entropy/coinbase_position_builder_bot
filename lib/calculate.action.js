...
module.exports = async ({
  ...
  //   pow(1 + config.apy, divide(1, divide(MS_PER_YEAR, action.msPassed))),
  //   1
  // );
  // simple interest rate conversion

  action.periodRate = divide(config.apy, divide(MS_PER_YEAR, action.msPassed));
  ...
  // and if this wasn't a rebuy/resell, add the expecting trading volume (funds)
  // if this buy was a rebey, the target was already adjusted the first time we bought
  // (before we sold and rebought)
  if (!isRebuy && !isResell) action.target = add(action.target, vol);
  ...
  // if (isResell) {
  //   // in a resell, we need to remove the funds from the target (acrued interest still applies)
  //   // NOTE: funds is negative
  //   action.target = add(action.target, action.funds);
  // }

  action.endValue = add(action.value, action.funds);
  action.realized = add(
    memory.lastLog.Realized,
    action.funds > 0 || isResell ? 0 : action.funds * -1
  );
