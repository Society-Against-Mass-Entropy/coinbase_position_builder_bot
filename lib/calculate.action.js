...
module.exports = (ticker, cb) => {
  log('calculateAction', memory.lastLog, ticker)
  const action = {}
  action.dateNow = new Date()
  action.dateLast = memory.lastLog.Time ? new Date(memory.lastLog.Time) : action.dateNow
  action.msPassed = action.dateNow - action.dateLast // milliseconds delta
  action.exchangeRate = Number(ticker.price)
  ...
  action.periodRate = pow(1 + config.apy, divide(1, divide(MS_PER_YEAR, action.msPassed))) - 1
  ...
  action.funds = action.diff > 0 ? config.vol * -1 : config.vol
  action.endValue = add(action.value, action.funds)
  action.realized = add(memory.lastLog.Realized, (action.funds > 0 ? 0 : action.funds * -1))
  // total input does not subtract when we take profit (that's realized profit)
  action.totalInput = add(memory.lastLog.TotalInput, (action.funds > 0 ? action.funds : 0))
