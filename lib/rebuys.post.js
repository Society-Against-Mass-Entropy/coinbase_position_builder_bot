...
module.exports = async (basePrice, maxFundsOverride) => {
  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    const detailLog = `${order.side} limit order for ${order.size} ${
      config.ticker
    } @${order.price} =$${order.funds} (${multiply(
      order.percentageDrop,
      100
    )}% drop)`;
    if (!orderResponse) {
  ...
  log.ok(
    `REBUY: Added ${addedOrders} limit buys totaling $${Math.abs(
      usedFunds
