...
module.exports = async (basePrice) => {
  ...
  const double = process.env.CPBB_REBUY_DOUBLING;
  let usedFunds = 0; // counter
  log.debug(
    `REBUY: setting up to ${process.env.CPBB_REBUY_MAX} limit orders, up to $${maxFunds}, for ${size} (${!double?'not ':''}doubling) ${config.ticker} @ ${process.env.CPBB_REBUY_AT}% drops`
  );
  ...
  for (let i = 0; i < process.env.CPBB_REBUY_MAX; i++) {
    ...
    // if we are doubling the btc size each drop point, do so (only if not the first order)
    if(double && i) size = multiply(size, 2);
