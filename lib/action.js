...
module.exports = async (opts) => {
  ...
  if (!order) {
    log.error(`order failed, retrying one time`);
    order = await processOrder(action);
    log.now("order:", order);
  }

  if (!order) {
    log.error('something is wrong with the API. Please file a bug report on https://github.com/jasonedison/coinbase_position_builder_bot/issues so we can investigate and add fault tolerance for this case.')
    return;
  }
  ...
  action.price = divide(Number(response.executed_value), Number(response.filled_size));
