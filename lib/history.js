...
const tsvAsObj = (headers, cells)=>{
  return headers.reduce((obj, nextKey, index) => {
    ...
    obj[nextKey] = textCols.includes(nextKey) ? cells[index] : Number(cells[index])
    return obj
  }, {})
}
const logData = (rows)=>{
...
module.exports = {
  ...
  headerRow: headerRow,
  last: ()=>{
    return dataRows.length > 1 ? tsvAsObj(headerCells, dataRows[dataRows.length - 1].split('\t')) : {}
  },
  log: ()=>{
    console.log('📜 History:')
    logData(dataRows)
